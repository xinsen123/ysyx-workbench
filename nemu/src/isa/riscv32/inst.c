...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
	...
	INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr	 , I, R(rd) = s->snpc; s->dnpc = (src1 + imm) & ~0x1);
	...
	INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl 	 , R, R(rd) = (word_t)src1 >> (src2 & 0x1f));
	...
	INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb	 , S, if(src1 + imm == 0xa00003f8) putchar(src2); else Mw(src1 + imm, 1, src2));
	...
	INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = (sword_t)(((int64_t)(sword_t)src1 * src2) >> 32));
	INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (word_t)(((uint64_t)src1 * (uint64_t)src2) >> 32));
	INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div 	 , R, R(rd) = src2 == 0 ? (word_t)-1 : (int64_t)(sword_t)src1 / (int64_t)(sword_t)src2);
	INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu 	 , R, R(rd) = src2 == 0 ? (word_t)-1 : src1 / src2);
	INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem 	 , R, R(rd) = src2 == 0 ? src1 : (int64_t)(sword_t)src1 % (int64_t)(sword_t)src2);
	INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu 	 , R, R(rd) = src2 == 0 ? src1 : (uint64_t)src1 % (uint64_t)src2);
